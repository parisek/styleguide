...
| `source_locale` | `'en_US'` | Locale code of the language the msgids are written in. Listed by `availableLocales()` and resolvable by `?locale=` even without a `.mo`; every lookup against it returns the msgid. Resolved last: a discovered catalogue wins an exact match, a prefix match, and a case-insensitive name clash, so adding a source locale never makes a previously unique prefix ambiguous — and one the catalogues would answer instead (a bare `en` beside `en_GB.mo`) is omitted from `availableLocales()` rather than offered unreachably. Refused at construction time when it is not a code `Router::whitelistLocale()` accepts. Only read when `translations_path` is set; `null` or `''` opts out. Added in the next minor release. |
...
| `source_locale` | no | `string\|null` | Locale code of the msgid language, default `en_US`; empty string or `null` opts out — see § Locale switching above |
