...
| `source_locale` | `'en_US'` | Locale code of the language the msgids are written in. Listed by `availableLocales()` and resolvable by `?locale=` even without a `.mo`; every lookup against it returns the msgid. A discovered catalogue of the same code wins. Only read when `translations_path` is set; `null` opts out. Added in the next minor release. |
