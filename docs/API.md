...
`/api/pages` and `/api/docs` inherit the identical additive `variants` and `has_default_fixture` fields (already true by construction — same `normaliseMetadata()`).
