...
| `styleguide_data(name = null, from = null)` | function | Parses and returns a `styleguide.data.yaml` / `styleguide.data-<name>.yaml` sidecar as a nested PHP array. Defaults to the CURRENTLY rendering component/page/doc's own directory; `from: '<kind>/<slug>'` reads another one's instead. See § `styleguide_data()` below for the full contract. |
...
- **Cross-component form:** `styleguide_data(from = '<kind>/<slug>')` resolves the sidecar in ANOTHER component/page/doc's directory — e.g. `styleguide_data(from = 'component/header')` from a page fixture. `<kind>` must be one of `component` / `page` / `doc` (the three `Router` accepts) and `<slug>` must match the same `/^[a-z0-9-]+$/` id rule as `<name>`; anything else is rejected with a `RuntimeException` before the filesystem is touched. The two arguments are orthogonal — `name` says WHICH set, `from` says WHOSE — so `styleguide_data('dark', 'component/header')` reads that component's named set. Both default to "mine", so every call written before `from` existed keeps its exact meaning.

  This replaced an earlier "no cross-component lookup" rule. The reasoning behind that rule — a fixture's demo data is its own business — held right up until several page fixtures needed to render the same chrome component. With no way to share, the only mechanism left was an `{% include %}` data partial, which cannot export variables and therefore grows into a single file carrying every consumer's data at once; one downstream project's reached 1147 lines. A named, greppable `from:` reference is a weaker coupling than that partial was, not a stronger one — the dependency is stated at the call site instead of being implied by an include chain.

  **Validation is whitelist-only, deliberately.** The resolved value is concatenated into a filesystem path, and a blacklist ("reject `..`") is the shape of check that keeps getting bypassed. Requiring exactly `<kind>/<slug>` from a closed set leaves no separator, traversal segment or absolute path expressible in either half.
