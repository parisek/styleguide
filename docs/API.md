...
- `<id>/styleguide.<variant>.twig` — OPTIONAL, zero or more. `<variant>` matches `[a-z0-9-]+`. Auto-discovered (no YAML required); when at least one exists, the SPA toolbar shows a variant switcher (its leading pill — `toolbar.variant_default`, "Vše"/"All" — selects the no-`?variant=` default view) and `?variant=<id>` becomes a valid query param on the SPA deep link and the render endpoint. Plain `styleguide.twig` remains the implicit default variant. The default view (no `?variant=`) stacks the default block plus every discovered variant, each under its own heading — see the render endpoint row above; selecting a variant still isolates it.
...
| `/styleguide/render/<kind>/<slug>` | Render endpoint — HTML document of a single component / page / doc in isolation (no SPA chrome); `<kind>` ∈ `component \| page \| doc \| foundations`. Accepts an additive `?theme=light\|dark` query param (whitelisted server-side, default `light`) — stamps `class="dark"` + `color-scheme: dark` on the rendered `<html>`. Also accepts an additive `?variant=<id>` query param (whitelisted server-side against `^[a-z0-9-]+$`) for `component \| page \| doc` kinds — resolves `styleguide.<id>.twig` in place of the default `styleguide.twig` when that file exists; absent, invalid, or unknown-but-well-formed values silently fall back to the default-view resolution below (never a 404), so a bookmarked deep link survives a deleted/renamed variant. Query-only — no cookie fallback, unlike `theme`. Composes independently with `?theme=`. **Default view (no `?variant=`, or one that doesn't resolve):** when the entry has discovered variants, the response stacks every block vertically — the default `styleguide.twig` body first (heading "Výchozí"/"Default" by locale), then each variant in discovery order, every block preceded by an `<h2 class="sg-variant-heading">` (its label) and, when the variant YAML supplies one, a `<p class="sg-variant-description">`. An entry with no discovered variants renders the single default block exactly as before (byte-identical, no `.sg-variant-*` markup). Requesting a specific, resolvable `?variant=` always isolates that one block — no headings, no stack. |
