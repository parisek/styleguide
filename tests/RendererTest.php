...
final class RendererTest extends TestCase
{
    ...
    /**
     * `multi`'s discovered variant records — mirrors what
     * `ComponentParser::discoverVariants()` returns for the `multi` fixture
     * (see ComponentParserTest::discovers_sibling_variant_files_ordered_by_filename).
     * Renderer learns these via the `variants` $config key (Styleguide::
     * dispatchRender() forwards `$meta['variants']` the same way it already
     * forwards `render` / `body_class`); RendererTest constructs it by hand
     * since these tests call Renderer directly, bypassing Styleguide.
     *
     * @return list<array{id:string,label:string,description:string}>
     */
    private function multiVariantRecords(): array
    {
        return [
    ...
    #[Test]
    public function stacks_default_and_every_variant_when_none_is_requested(): void
    {
        $html = $this->renderer->render('component', 'multi', [
            'project' => ['name' => 'TestProject'],
            'iframe' => [],
            'variants' => $this->multiVariantRecords(),
        ], 'en');

        // All three bodies present…
        self::assertStringContainsString('multi--demo', $html);
        self::assertStringContainsString('multi--dark-bg', $html);
        self::assertStringContainsString('multi--secondary', $html);
        // …the foo_bar phantom sibling (invalid id, never discovered) never
        // surfaces even though its file sits in the same directory.
        self::assertStringNotContainsString('never discovered', $html);

        // Three headings, in stack order: default first, then discovery
        // order (dark-bg before secondary, same as the switcher).
        // Count the actual <h2> tags, not the bare class name — it also
        // appears twice in render-cell.twig's own <style> block (the
        // heading rule plus the first-child override).
        self::assertSame(3, substr_count($html, '<h2 class="sg-variant-heading">'));
        preg_match_all('/<h2 class="sg-variant-heading">([^<]*)<\/h2>/', $html, $matches);
        self::assertSame(['Default', 'dark-bg', 'Secondary style'], $matches[1]);

        // Headings precede their own body, and the default section precedes
        // the variant sections (stack order, not alphabetical).
        $posDefaultHeading = strpos($html, 'Default');
        $posDefaultBody = strpos($html, 'multi--demo');
        $posDarkHeading = strpos($html, 'dark-bg</h2>');
        $posDarkBody = strpos($html, 'multi--dark-bg');
        $posSecondaryHeading = strpos($html, 'Secondary style');
        $posSecondaryBody = strpos($html, 'multi--secondary');
        self::assertLessThan($posDefaultBody, $posDefaultHeading);
        self::assertLessThan($posDarkBody, $posDarkHeading);
        self::assertLessThan($posSecondaryBody, $posSecondaryHeading);
        self::assertLessThan($posDarkHeading, $posDefaultHeading, 'default section renders first');
        self::assertLessThan($posSecondaryHeading, $posDarkHeading, 'variants render in discovery order');
    }

    #[Test]
    public function stacked_view_renders_description_only_when_the_variant_has_one(): void
    {
        $html = $this->renderer->render('component', 'multi', [
            'project' => ['name' => 'TestProject'],
            'iframe' => [],
        ...
        // secondary carries a YAML description -> its <p> renders.
        self::assertStringContainsString(
            '<p class="sg-variant-description">Tuned for a secondary-toned surface.</p>',
            $html,
        );
        // dark-bg has no description -> exactly one actual <p> tag (secondary's)
        // in the whole stack, never an empty one for dark-bg. Counts the <p>
        // tag specifically, not the bare class name (also present once in
        // render-cell.twig's gated .sg-variant-description CSS rule).
        self::assertSame(1, substr_count($html, '<p class="sg-variant-description">'));
    }

    #[Test]
    public function requesting_a_variant_still_isolates_it_with_no_headings(): void
    {
        $html = $this->renderer->render('component', 'multi', [
            'project' => ['name' => 'TestProject'],
            'iframe' => [],
        ...
        self::assertStringContainsString('multi--secondary', $html);
        self::assertStringNotContainsString('multi--demo', $html);
        self::assertStringNotContainsString('multi--dark-bg', $html);
        self::assertStringNotContainsString('sg-variant-heading', $html);
        self::assertStringNotContainsString('sg-variant-section', $html);
    }

    #[Test]
    public function entry_without_variants_never_gets_a_variant_section(): void
    {
        $html = $this->renderer->render('component', 'sample', [
            'project' => ['name' => 'TestProject'],
            'iframe' => [],
