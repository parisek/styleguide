...
**Rows vs grid layout.** A segmented toolbar control next to the preset dropdown (visible only while the grid is active) switches how tiles flow: "grid" (the default) lays them out side by side in as many auto-fit columns as fit the canvas width; "rows" stacks one tile per row. The choice is remembered across visits (`localStorage`, key `sg-variant-layout`).
