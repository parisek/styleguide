...
- **`?variant=<id>` on the render endpoint** resolves the auto-discovered `styleguide.<id>.twig` sibling in place of the default `styleguide.twig`, for `component`/`page`/`doc` kinds. Whitelisted server-side against `^[a-z0-9-]+$` (`Router::whitelistVariant()`) on both the render endpoint and the SPA-shell deep links (`/component/<slug>`, `/page/<slug>`, `/doc/<slug>`) so `Router::synthesizeEmbeddedRoute()` can forward it across the iframe-embed swap. Absent, invalid, or unknown-but-well-formed values fall back silently to the default `styleguide.twig` → `<slug>.twig` chain — never a 404 — so a bookmarked deep link survives a deleted/renamed variant file. Query-only, no cookie fallback (unlike `theme`): a variant is scoped to one deep link, not a visitor preference worth persisting, so an in-iframe native navigation to a link without its own `?variant=` loses it on the swap — an accepted, documented gap. Composes independently with `?theme=`. The SPA toolbar now shows a "Default"/`<variant label>` segmented switcher whenever the active entry has discovered variants — deep-linkable via the SPA's own `?variant=` query param, silently reset on navigation to a different entry unless the incoming URL itself carries a valid one for that entry.

- **Stacked default view for file-convention variants, plus optional per-variant descriptions.** With no `?variant=` (or one that doesn't resolve), the render endpoint now stacks every block vertically for an entry with discovered variants — the default `styleguide.twig` body first (heading "Výchozí"/"Default" by locale), then each variant in discovery order, every block preceded by an `<h2 class="sg-variant-heading">` and, when supplied, a `<p class="sg-variant-description">`. Requesting a specific variant (toolbar pill or `?variant=<id>`) is unchanged — still isolates just that one block, no headings. An entry with no discovered variants renders exactly as before (BC: byte-identical, no `.sg-variant-*` markup or CSS). The YAML `variants:` map now accepts either the original plain-string shape (label only) or a map with optional `label`/`description` keys; a non-string/non-map value degrades to the id as label, same as an absent entry — never throws. `ComponentParser`'s discovered variant records gain an additive `description` field (`''` unless supplied). The SPA toolbar's leading pill is relabeled `toolbar.variant_default` "Vše"/"All" (was "Výchozí"/"Default") to describe what it now shows.
