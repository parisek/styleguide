...
- **Variant grid — every variant as its own preview screen, tiled to fit the canvas (SPA-only; prototype).** The toolbar pill switcher is gone, and so is the short-lived server-side stacked view: the render endpoint's `?variant=` semantics are unchanged from the file-convention variants feature above (no `?variant=` → the default `styleguide.twig` body only, single block, no headings; `?variant=<id>` → isolate that one block). Instead, whenever the SPA's current entry has discovered variants and no `?variant=` is selected, the preview area becomes a responsive CSS grid (`repeat(auto-fit, minmax(min(420px, 100%), 1fr))`) of independent `<iframe>` tiles — the default fixture first, then each discovered variant in order — tiles flowing side by side and wrapping below as they fit the canvas width. Each tile gets a slim header (the variant's label, plus its YAML `description` when supplied) and its own auto-height iframe (content-driven via the same same-origin `ResizeObserver` technique as the classic single preview, with a modest fallback height). A deep-linked `?variant=<id>` still shows the classic single preview of just that variant (an unknown/removed id falls back to the grid, not a 404 and not a stale "selected" pill — there is no pill any more); the responsive-width preset dropdown, drag-to-resize, and the on-demand accessibility check are single-preview-only in this pass — the a11y check button is disabled (with a title hint) rather than hidden while the grid is active. What (if anything) belongs in the toolbar to indicate or control grid mode is this prototype's open question.
